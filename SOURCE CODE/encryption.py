...
class Aes_enc:
    ...
    def enc_file(self, filename, passkey):
        ...
        with open(filename + ".aes", 'wb') as fo:
            fo.write(enc)

    def dec_file(self, filename, passkey):
        with open(filename+".aes", 'rb') as fo:
            ciphertext = fo.read()
...
class Bfs_enc():
    ...
    def enc_file(self, filename, passkey):
        with open(filename, 'rb') as fo:
            plaintext = fo.read()
        enc = self.encrypt(plaintext, passkey)
        with open(filename + ".bfs", 'wb') as fo:
            fo.write(enc)

    def dec_file(self, filename, passkey):
        with open(filename+".bfs", 'rb') as fo:
            ciphertext = fo.read()
        dec = self.decrypt(ciphertext, passkey)
        with open(filename, 'wb') as fo:
            fo.write(dec)
...
class Sla_enc():
    ...
    def enc_file(self, filename, passkey, nonce):
        ...
        with open(filename + ".sla", 'wb') as fo:
            fo.write(enc)

    def dec_file(self, filename, passkey, nonce):
        with open(filename + ".sla", 'rb') as fo:
            ciphertext = fo.read()
...
class File_Enc(Enc):
    ...
    def enc(self, filename, passkey, nonce):
        ...
        self.bfs.enc_file(filename+".sla", passkey)
        files.append(filename+".sla")
        self.aes.enc_file(filename+".sla.bfs", passkey)
        files.append(filename+".sla.bfs")
        self.clean_files(files)

    def dec(self, filename, passkey, nonce):
        files=[]
        self.aes.dec_file(filename+".sla.bfs", passkey)
        files.append(filename+".sla.bfs.aes")
        self.bfs.dec_file(filename+".sla", passkey)
        files.append(filename+".sla.bfs")
